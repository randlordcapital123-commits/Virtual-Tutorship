// ==================== PASSWORD PROTECTION ====================
const ADMIN_PASSWORD = 'admin123';  // default password – change as needed
...
function checkAdminPassword() {
    ...
    const entered = prompt('Please enter admin password:');
...
function showAdminPanel() {
    ...
    // Re-render products in case they changed
    renderAdminProducts();
}
...
document.querySelector('.nav-admin').addEventListener('click', function(e) {
    e.preventDefault();  // prevent hash navigation
    checkAdminPassword();
});

// ==================== ADD LOGOUT BUTTON INSIDE ADMIN ====================
// We'll add a logout button dynamically to the admin panel
// after the admin section loads (in the HTML we'll add a placeholder)
// Better: we'll insert a logout button into the admin panel header.
...
// ==================== ADD LOGOUT BUTTON ====================
// Insert logout button after the admin panel header
const adminSection = document.getElementById('admin');
const adminHeader = adminSection.querySelector('.section-header');
...
// Place it inside the admin container, after the section-header
const container = adminSection.querySelector('.container');
container.insertBefore(logoutBtn, container.children[1]); // after the section-header

// ==================== INIT ====================
updateLogoUI();
renderPublicProducts();
// Admin panel is hidden by default; only render products if already shown? 
// We'll render when showing admin panel.